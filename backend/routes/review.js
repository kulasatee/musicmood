...
//     try{
//         const [reviews, columns] = await pool.query("SELECT * FROM reviews");
...
//get review
router.get("/reviews/:review_id", async function (req, res, next) {
    try{
        const [reviews, columns] = await pool.query("SELECT * FROM reviews WHERE review_id = ?", [req.params.review_id]);
        return res.json(reviews);
    } catch (err) {
        return next(err);
    }
